...
Space Fan Art is a working API prototype with two different architectures: monlithic and microservice. It was created with Flask REST-Plus to showcase the OWASP Top Ten Proactive Controls. In the monolithic architecture, users have to register to get an authentication key, which allows them to download images from NASA and also contribute with our database by uploading pictures. In both architectures we provide a demo key, so users can consume the API without being registered. The main difference between these two architectures is where the images come from: in the monolithic architecture, they come from a local database containing [365 images from NASA](https://github.com/alicevillar/sfa_api/blob/main/APOD_365). In the microservice architecture, SFA-API and [APOD](https://github.com/nasa/apod-api)(one of the most famous NASA's Open API) are integrated so the pictures will come directly from  APOD.   
