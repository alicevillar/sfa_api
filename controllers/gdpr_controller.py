...
class DeleteData(Resource):
    # ex: email = request.form.get('email') # gets email in the request / checks in the database
    @gdpr_namespace.expect(gdpr_model_request)
    def delete(self):
        user_information_to_delete = request.get_json()
        # user = request.get_json()  # this json contains email and password. ex: {"email": "email@mail.com", "password": "432432432543"}
        try:
        ...
        try:
            ...
            print("Invalid email")
            return {"Error:": "Invalid email"}, 401  # Unauthorised
        ...
        if check_password_hash(hashed_password, user_information_to_delete["password"]):
            ...
            if deleted_row_count >0:
                return {"Info": "Your data was succefully deleted from our database"}, 200  # Success
            else:
                return {"Error": "System could not delete the user"}, 422 # Unprocessable
        return {"Error:": "Invalid password "}, 401 # Unauthorised
